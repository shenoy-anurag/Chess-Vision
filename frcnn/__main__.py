...
# import statistics
# from datasets import voc
# from models import faster_rcnn, vgg16, math_utils, anchors
# import utils
# import visualize

import frcnn.statistics as statistics
from frcnn.datasets import voc
from frcnn.models import faster_rcnn, vgg16, math_utils, anchors
...
def evaluate(model, eval_data=None, num_samples=None, plot=False, print_average_precisions=False):
    ...
    precision_recall_curve = statistics.PrecisionRecallCurveCalculator()
    i = 0
...
def train(model):
    ...
    # training_data = voc.Dataset(dir=options.dataset_dir, split=options.train_split, augment=not options.no_augment,
    ...
    for epoch in range(1, 1 + options.epochs):
        ...
        stats = statistics.TrainingStatistics()
...
def predict_all(model, split):
    ...
    for sample in iter(dataset):
        ...
        _predict(model=model, image_data=sample.image_data, image=sample.image, show_image=False,
                 output_path=output_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset-dir", metavar="dir", action="store", default="dataset/chess-pieces-dataset",
                        help="Chess dataset directory")
    ...
    parser.add_argument("--optimizer", metavar="name", type=str, action="store", default="sgd",
                        help="Optimizer to use (\"sgd\" or \"adam\")")
    parser.add_argument("--learning-rate", metavar="value", type=float, action="store", default=1e-3,
                        help="Learning rate")
    parser.add_argument("--clipnorm", metavar="value", type=float, action="store", default=0.0,
                        help="Gradient norm clipping (use 0 for none)")
    parser.add_argument("--momentum", metavar="value", type=float, action="store", default=0.9, help="SGD momentum")
    parser.add_argument("--beta1", metavar="value", type=float, action="store", default=0.9,
                        help="Adam beta1 parameter (decay rate for 1st moment estimates)")
    parser.add_argument("--beta2", metavar="value", type=float, action="store", default=0.999,
                        help="Adam beta2 parameter (decay rate for 2nd moment estimates)")
    ...
    parser.add_argument("--custom-roi-pool", action="store_true",
                        help="Use custom RoI pool implementation instead of TensorFlow crop-and-resize with max-pool (much slower)")
    parser.add_argument("--detector-logits", action="store_true",
                        help="Do not apply softmax to detector class output and compute loss from logits directly")
    ...
    parser.add_argument("--debug-dir", metavar="dir", action="store",
                        help="Enable full TensorFlow Debugger V2 logging to specified directory")
